...
IMAGES_DIR = "images"  # optional banners (s1.png ... s12.png). App works without them.

# ============================================================
# 12 CONCISE, ROLE-NEUTRAL SINGAPORE WORKPLACE SCENARIOS
# ============================================================
SCENARIOS = [
    ...
    {"key":"s10","title":"The Change Rollout",
     ...
     "opts":[
       ...
       ("Set up Q&A and a feedback loop.", "C"),
...
MICRO_PRACTICES = {
    ...
    "R": "🔍 After a key meeting, host a 5-min sense-making pause: 'What did we learn? What assumptions surfaced?' Capture 1 line per person.",
...
def generate_result_card(code, scores, width=1080, height=1920):
    ...
    # gradient background
    img = Image.new("RGB", (width, height), theme)
    ...
    y += 120
    d.text((x, y), meta["desc"], font=h1_f, fill=(65,65,65))
    y += 120

    # tag pills
    pill_x = x
    for tag in meta.get("tags", [])[:3]:
        draw_pill(d, pill_x, y, tag, tag_f, fill=(60,60,60))
        pill_x += 260
    y += 110

    d.line([(x, y), (width - pad - 52, y)], fill=(220,220,220), width=4)
    y += 40

    # mini bar chart
    d.text((x, y), "Your Boundary Compass", font=h1_f, fill=(30,30,30))
    y += 64
...
def scenario_ui(i):
    ...
    choice = st.radio("What would you most likely do?",
                      opts, index=default, label_visibility="collapsed", key=f"q_{i}")
...
def results_ui():
    ...
    # Result card and download button
